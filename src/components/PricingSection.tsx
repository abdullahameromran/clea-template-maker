...
const PricingSection = ({ user }: PricingSectionProps) => {
  ...
  const handleCheckout = async (planCode: "pro" | "business") => {
    ...
    try {
      const checkoutUrl = await createStripeCheckoutSession({
        planCode,
        billingCycle: "monthly",
        successPath: "/?checkout=success#pricing",
        cancelPath: "/?checkout=cancelled#pricing",
      });
  ...
  const handleManageBilling = async () => {
    ...
    try {
      const portalUrl = await createStripePortalSession("/#pricing");
  ...
  return (
    <section id="pricing" className="relative overflow-hidden py-20 px-6 bg-card border-y border-border">
      ...
      <div className="relative max-w-7xl mx-auto">
        <div className="text-center mb-10">
          ...
          <h2 className="font-display text-4xl md:text-5xl font-bold text-foreground mb-4">
            ...
            <br />
            Full Plans in a Popup
          </h2>
          <p className="text-muted-foreground text-lg max-w-3xl mx-auto">
            Keep the page clean, check your current usage here, and open the full pricing popup only when you want to compare plans or upgrade.
          </p>
        </div>
      ...
      <Dialog open={plansOpen} onOpenChange={setPlansOpen}>
        <DialogContent className="max-w-6xl w-[96vw] max-h-[90vh] overflow-hidden border-border bg-card p-0">
          <div className="max-h-[90vh] overflow-y-auto p-6 md:p-8">
            <DialogHeader className="pr-8">
              <DialogTitle className="font-display text-3xl text-foreground">
                Choose the plan that matches your invoice volume
              </DialogTitle>
              <DialogDescription className="text-sm leading-6 text-muted-foreground">
                Compare all app plans here, then upgrade only when you need more invoices, faster throughput, or advanced automation.
              </DialogDescription>
            </DialogHeader>
